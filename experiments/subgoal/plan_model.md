...
### 6.2 v1_subgoal 系统深度分析

**v1_subgoal 使用的模型：video-to-action (AVDC) — 非 Wan2.2**

**来源：** [github.com/video-to-action/video-to-action-release](https://github.com/video-to-action/video-to-action-release) (ICLR 2025 Spotlight, "Grounding Video Models to Actions through Goal Conditioned Exploration")

| 属性 | 值 |
|------|-----|
| 架构 | Factorized 3D U-Net (空间+时间分离卷积) |
| 参数量 | ~201M |
| 扩散类型 | DDPM (非 flow matching, 非 DDIM) |
| 分辨率 | **128×128** (固定，不可调) |
| 输出帧数 | **7 帧/次** |
| 条件输入 | 首帧 RGB tensor + 语言指令 (text) |
| 训练域 | **LIBERO 仿真环境** (8 tasks) |
| 推理设置 | `var_temp=1.0`, `is_ddim_sampling=False` |

**代码路径 (v1_subgoal 集成)：**
- 模型加载: `lb_get_video_model_gcp_v2(ckpts_dir, milestone=180000, flow=False)`
- 推理: `model.forward(img_tensor, tasks_str)` → `List[(T=7, 3, 128, 128)]`
- Subgoal 提取: 取第 `subgoal_interval-1` 帧 (默认第 7 帧) 作为下一轮输入
- 脚本: `vm_subgoal_generator.py` (subprocess) / `verify_libero_quality.py` (验证)

**v1_subgoal 的问题（为什么不行）：**
1. **128×128 分辨率过低** — 比 Wan2.2 (704×1280) 小 55x，丢失精细空间信息
2. **迭代累积误差** — 每轮预测 7 帧取末帧，20 subgoal = 19 次迭代，误差指数积累
3. **Domain mismatch** — LIBERO 仿真 vs RoboTwin 双臂 aloha，场景完全不同
4. **实际只用前 2 帧** — `use_vid_first_n_frames=2`，说明后续帧质量不可靠
5. **延迟过高** — 19 次 DDPM forward × 0.5-2s = 10-40s
6. **无长程一致性** — 模型没有全局视野，每轮独立 7 帧预测
7. **无 CFG 控制** — 直接 `model.forward()`，无 guide_scale / negative prompt

**验证脚本：** `verify_libero_quality.py` — 已就绪，待在 GPU 机器运行

**结论：AVDC 本质是短程视频预测的迭代拼接，128×128 + LIBERO 域训练，不具备 RoboTwin 场景的长程规划能力。**
...
**基座模型：Wan 2.1 I2V 14B** — 与我们的 Wan 2.2 TI2V 5B 同源但不同规模

| 属性 | 值 |
|------|-----|
| 论文 | "Large Video Planner Enables Generalizable Robot Control" (Dec 2025) |
| 基座 | **Wan 2.1 I2V 14B** (非 TI2V 5B，参数量 3x) |
| 方法 | Diffusion Forcing (每 segment 不同噪声: history=0, future=random) |
| 训练分辨率 | **832×480**, 49 帧 |
| 训练数据 | **LVP-1M** (1.4M clips): AgiBot 863k, DROID 192k, Bridge 25k, Language-Tables 71k, Ego4D+Epic-Kitchens |
| 训练资源 | 128×H100, 14 天 |
| 长程能力 | 迭代 V2V 扩展，超出训练 horizon |
| 推理延迟 | 数分钟/plan (A100) |
...
**优势：**
- 82% 训练数据是 manipulation 任务（与 RoboTwin 最相关的域）
- Diffusion Forcing 天然支持因果长程生成，无需自回归拼接
- 832×480 高分辨率，远超 AVDC 的 128×128
- 开源权重 + 代码，可直接实验

**障碍：**
- **基座不兼容** — Wan 2.1 I2V 14B ≠ Wan 2.2 TI2V 5B，权重不可互换
- **推理成本高** — 14B 模型 + 完整去噪，数分钟/plan，不适合实时部署
- **非 aloha 域** — 训练数据主要是单臂 (DROID/Bridge)，非双臂 aloha
- **需要额外下载** — Wan 2.1 I2V 14B base (~28GB) + LVP LoRA weights

**结论：LVP 作为 subgoal planner 可行但重量级。更实际的路径是借鉴其 Diffusion Forcing 方法，移植到我们已有的 Wan 2.2 / vidar 上。**

**关键价值：** 证明了在 Wan 上微调做 robot planning 是可行的。Diffusion Forcing 方法可直接移植到 Wan 2.2 / vidar。
