...
def get_model(usr_args):  # from deploy_policy.yml and eval.sh (overrides)
    ...
    # Support version parameter from usr_args
    version = usr_args.get("version", None)
    model = AR(usr_args=usr_args, version=version)
    return model
...
def eval(TASK_ENV, model, observation):
    ...
    first_frame_obs_b64 = b64encode(img.numpy().tobytes()).decode("utf-8")
    
    if model.use_libero_subgoal and len(model.obs_cache) == 0:
        # 只在第一次调用时生成subgoals（obs_cache为空表示这是第一帧）
        print("Generating subgoals based on first frame observation and instruction...")
        model.current_subgoals = model.get_subgoals(first_frame_obs_b64, model.prompt)
        print(f"Generated {len(model.current_subgoals)} subgoals with interval {model.subgoal_interval} frames")
